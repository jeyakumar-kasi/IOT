...
// Seperate Serial communicator for reading input for Alert System from Python.
// ### Must SYNC with 'char' given in Python file ###
const int ALERT_BAURD_RATE = 115200;
const char HUMAN_DETECTION_CHAR = 'H'; 
...
void alert(int secs) { // secs
  times = secs <= 0 ? 999999 : secs * 2;  
  // Sound a pulsing alarm for 3 seconds
...
void _setup() {
  // Serial.begin(9600);
  // Start serial communication at 115200 baud for Alert
  Serial.begin(ALERT_BAURD_RATE);    
  while (! Serial);
