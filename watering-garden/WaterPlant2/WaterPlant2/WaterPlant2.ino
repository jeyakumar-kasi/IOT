...
const long motorRunningTime = (long) 30 * 60 * 1000; //(long) 2 * 60 * 60 * 1000; // in millis (2 Hrs)
...
bool isMotorRunning = false;  // Motor's default state.
bool isInit = true; // Is the board Restarted?
...
void setup() {
  ...
  pinMode(motorOnPin, INPUT_PULLUP);
...
void loop() {
    byte buttonState = digitalRead(motorOnPin);
    if (isInit || (isMotorRunning && buttonState == HIGH)) {
      // By default, always stop the Motor at first time for safety reasons.
      isInit = false; 

      // Manually stop the Motor
      stopMotor();
    } else if (isMotorRunning == false) {
      // Check need to start or not?
      if (buttonState == LOW) { // Button is pressed
        // Manually start the Motor
        startMotor();
      } else {
        // checkRTCStatus();
