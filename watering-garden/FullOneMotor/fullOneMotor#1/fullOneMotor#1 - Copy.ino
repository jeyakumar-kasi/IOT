...
const String runEveryDayAt = "23:59:00"; // HH:MM:SS
const long intervalTime = (long) 1 * 60 * 1000; //1 * 24 * 60 * 60 * 1000; // in millis (2 days)
const long motorRunningTime = (long) 5 * 1000; //2 * 60 * 60 * 1000; // in millis (2 Hrs)
...
DateTime getNextRunDateTime()
{
  ...
  if (lastRanStateStr && lastRanStateStr != "") {
    ...
    if (lastRanStateLevel < (float) lastRanThresholdPercent) {
      ...
      Serial.println("Calculating the next run time...");
      ...
      printDateTime(lastRanDateTime);
      printTimeSpan(millisToTimeSpan(intervalTime));
...
{
  Serial.print(t.hours()); Serial.print(":"); Serial.print(t.minutes()); Serial.print(":"); Serial.print(t.seconds());
}
...
void setup() {
  ...
  // RTC
  checkRTCStatus();
  rtc.adjust(DateTime(2023, 1, 7, 23, 59, 0)); //!
